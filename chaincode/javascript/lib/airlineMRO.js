  ...
  // Adding entires for verifying chaincode works
    async initLedger(ctx) {
        ...
        for (let i = 0; i < test.length; i++) {
            ...
            await ctx.stub.putState((i+1).toString(), Buffer.from(JSON.stringify(test[i])));
    ...
    async registerUser(ctx, user) {
        ...
        const compositeKey = await ctx.stub.createCompositeKey(relations, [user.type.toString(), user.company.toString(), user.username.toString()]);  //  create company~user relation unique key
        ...
        await ctx.stub.putState(compositeKey, Buffer.from(JSON.stringify(user)));    //  Store composite key relation for user
